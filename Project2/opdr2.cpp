...
void SaltPepperCalc(byte* a, int n){
	...
	int average = sum / (n*n);
	a[midPix] = (byte)average;
}

void FilterSaltPepper(int imageWidth, int imageHeight, int windowSize, byte* sourceScan, byte* destinationScan, int sourceStride, int destinationStride){
	int borderSize = (windowSize - 1) / 2;//get border size. This is size from after the middle point to the outside radius
	byte* red = new byte[windowSize * windowSize]; //create array to hold red pixels 
	byte* green = new byte[windowSize * windowSize]; //create array to hold green pixels 
	byte* blue = new byte[windowSize * windowSize]; //create array to hold blue pixels 
	...
	for (int i = borderSize; i < (imageHeight - borderSize); i++) //height loop (exclude the border)
	{
		for (int j = borderSize; j < (imageWidth - borderSize); j++) //width loop (exclude the border)
		{
			...
			SaltPepperCalc(red, windowSize); //sort! NOTE: any sorter based on sizeof won't work because of possible 0 byte!!
			SaltPepperCalc(green, windowSize);
			SaltPepperCalc(blue, windowSize);
...
void FilterMaximum(int imageWidth, int imageHeight, int windowSize, byte* sourceScan, byte* destinationScan, int sourceStride, int destinationStride){
	...
	for (int i = borderSize; i < (imageHeight - borderSize); i++) //height loop (exclude the border)
	{
		for (int j = borderSize; j < (imageWidth - borderSize); j++) //width loop (exclude the border)
		{
			...
			MaxCalc(red, windowSize); //sort! NOTE: any sorter based on sizeof won't work because of possible 0 byte!!
			MaxCalc(green, windowSize);
			MaxCalc(blue, windowSize);
